...
build:
	GOARCH=amd64 GOOS=darwin go build -o ${BINARY_NAME}-darwin-amd64.app ${SRC}/main.go
	GOARCH=amd64 GOOS=linux go build -o ${BINARY_NAME}-linux-amd64 ${SRC}/main.go
	GOARCH=amd64 GOOS=windows go build -o ${BINARY_NAME}-windows-amd64.exe ${SRC}/main.go
